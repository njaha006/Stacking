...
def process_train_test(train_file, test_file, balanced_all=False):
    ...
    if balanced_all:
        # TODO
        print('processing the balanced training set...')
        trainDoc_raw, trainClass, augmentDoc_raw, augmentClass = Balance_Data.read_balanced_training()
...
def process_test(file_name):
    ...
    # Processes English Test corpus and validate the data
    test_data = Read_Data.read_csv(file_name, separator='\t')
    testDoc_raw, testClass = Read_Data.process_data(test_data)
...
def process_unlabelled_test(file_name):
    ...
    # Processes English Test corpus and validate the data
    test_data = Read_Data.read_csv(file_name, separator='\t')
    tweet_ids, testDoc_raw = Read_Data.process_unlabelled_data(test_data)
